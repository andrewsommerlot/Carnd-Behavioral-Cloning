...
y_trim_a = [y for X, y in zip(augmented_images, augmented_measurements) if np.abs(y) > .2]
X_trim_a = [X for X, y in zip(augmented_images, augmented_measurements) if np.abs(y) > .2]

# absolute value less than 0.3
y_trim_b = [y for X, y in zip(augmented_images, augmented_measurements) if np.abs(y) < .2]
X_trim_b = [X for X, y in zip(augmented_images, augmented_measurements) if np.abs(y) < .2]
...
random_index = random.sample(range(len(y_trim_b)), 3000)
...
y_trim_e = [y for X, y in zip(X_trim_c, y_trim_c) if np.abs(y) > .5]
X_trim_e = [X for X, y in zip(X_trim_c, y_trim_c) if np.abs(y) > .5]
...
# combine them, add 2 ds and 5 es
y_trim = y_trim_c + y_trim_d + y_trim_d + y_trim_e + y_trim_e + y_trim_e + y_trim_e + y_trim_e
X_trim = X_trim_c + X_trim_d + X_trim_d + X_trim_e + X_trim_e + X_trim_e + X_trim_e + X_trim_e 
#====================================================================
...
X_train1, X_valid, y_train1, y_valid = train_test_split(X_train, y_train, test_size=0.1, shuffle = True, random_state=2490)
...
X_train_b, y_train_b = next(datagen_train.flow(X_train1, y_train1, batch_size=9))
X_valid_b, y_valid_b = next(datagen_valid.flow(X_valid, y_valid, batch_size=1))
cv2.imwrite("test1.jpg", X_train_b[0])

for i in range(0,9):
    cv2.imwrite('test_' + str(i) + '.jpg', X_train_b[i])
...
#    a, b = x
#    cv2.imwrite('test_' + str(x[1][1]) + '.jpg', a)
...
# source [https://stackoverflow.com/questions/43915482/how-do-you-create-a-custom-activation-function-with-keras]
from keras import backend as K
from keras.utils.generic_utils import get_custom_objects

def cube_spread(x, alpha = 1):
    return ((x*alpha)**3)

get_custom_objects().update({'cube_spread': Activation(cube_spread)})
...
model.add(Conv2D(24, (5, 5), strides = (2,2), activation = "elu", kernel_regularizer = l2(.001)))
model.add(Conv2D(36, (5, 5), strides = (2,2), activation = "elu", kernel_regularizer = l2(.001)))
model.add(Conv2D(48, (3, 3), activation = "elu", kernel_regularizer = l2(.001)))
model.add(MaxPooling2D(pool_size=(2, 1), strides=None, padding='same'))
model.add(Conv2D(64, (3, 3), activation = "elu", kernel_regularizer = l2(.001)))
model.add(MaxPooling2D(pool_size=(2, 1), strides=None, padding='same'))
model.add(Conv2D(64, (3, 3), activation = "elu", kernel_regularizer = l2(.001)))
model.add(Flatten())
model.add(Dense(100, kernel_regularizer = l2(.001), activation = 'elu'))
model.add(Dense(25,  kernel_regularizer = l2(.001), activation = 'elu'))
model.add(Dense(10, kernel_regularizer = l2(.001), activation = "elu"))
model.add(Dense(1))
adam = Adam(lr = 0.001)
